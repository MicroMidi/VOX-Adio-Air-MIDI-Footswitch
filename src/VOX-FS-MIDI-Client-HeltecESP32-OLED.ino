/**
   ...
   I adjusted it for the "VOX Adio Air" device and the Heltec ESP32 WiFi Kit. This version supports four buttons:
   ...
   Optionally an analog volume pedal can be attached to the solution in order to controö the master volume of the VOX device
...
byte adioVolume[] = { 0xF0, 0x42, 0x30, 0x00, 0x01, 0x41, 0x41, 0x04, 0x04, 0x00, 0x00, 0xF7 };
...
// Connect to the VOX BLE-MIDI service
BLEMIDI_CREATE_INSTANCE("Adio Air GT MIDI", MIDI)       //Connect to a specific name server
...
void setup()
{
  ...
  MIDI.setHandleSystemExclusive([](byte * SysExArray, unsigned SysExSize) {
    ...
    if (adioBank > 7) {
      ...
      if (MIDIVolume >= 0) {
        adioVolume[9] = MIDIVolume;
        MIDI.sendSysEx(sizeof(adioVolume), adioVolume, true);
      }
#endif				 
    }
  });
...
void loop()
{
  ...
  if (isConnected) {
    ...
    if (FirstRun == true && (millis() - t0) > 200) {
      ...
      Heltec.display->drawString(0, 20, "VOX connected");
      Heltec.display->display();
  ...
  // Loop for disconnected device
    ...
    if ((millis() - tSearchDevice) > 1000) {
      ...
      if (FirstRunDisconnect == true) {
        ...
        Heltec.display->setTextAlignment(TEXT_ALIGN_LEFT);
        Heltec.display->setFont(ArialMT_Plain_16);
        Heltec.display->drawString(40, 10, "VOX");
...
#ifdef PIN_VOLUME
  if (MIDIVolume >= 0)
    Heltec.display->drawProgressBar(0, 50, 120, 10, map(MIDIVolume, 0, MAX_VOLUME, 0, 100));
#endif
...
void ReadVolumePedal()
{
  ...
  int nMappedValue = map(nCurrentPotValue, 0, 4095, 0, MAX_VOLUME);    // Map the value to 0-100
